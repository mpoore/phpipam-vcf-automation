...
def do_deallocate_ip(self, auth_credentials, cert):
    ...
    # Process all deallocations in inputs
    deallocationResult = []
    for deallocation in self.inputs["ipDeallocations"]:
        deallocationResult.append(deallocate(self.inputs["resourceInfo"], deallocation, baseUri, token, cert))

    assert len(deallocationResult) > 0
    return {
        "ipDeallocations": deallocationResult
    }

def deallocate(resource, deallocation, baseUri, token, cert):
    # Process the IP address in a dallocation request
    rangeId = deallocation["ipRangeId"]
    ipAddr = deallocation["ipAddress"]
    resourceId = resource["id"]

    logging.info(f"deallocating IP address {ipAddr} from range {rangeId}")

    # Execute request
    deallocateUri = f'{baseUri}/addresses/{ipAddr}/{rangeId}/'
    requests.delete(deallocateUri, headers=token, verify=cert)
